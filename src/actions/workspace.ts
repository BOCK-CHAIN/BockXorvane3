...
export const createWorkspace = async (name: string) => {
  try {
    ...
    if (authorized?.subscription?.plan !== "NONE") {
      const workspaceExists = await client.workSpace.findFirst({
        where:{
          userId: user.id,
          name
        }
      })
      if(workspaceExists) return { status: 400, data: "Workspace already exists with this name." }
      ...
      if (workspace) {
        return { status: 201, data: "Workspace Created. Please wait or refresh to see the workspace." };
      }
    }
    return {
      status: 401,
